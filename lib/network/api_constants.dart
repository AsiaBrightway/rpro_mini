
const String kParamAuthorization = "Authorization";

const String kEndPointColors = "/api/colors";
const String kEndPointSizes = "/api/sizes";
const String kEndPointBrands = "/api/brands";
const String kEndPointProducts = "/api/products";
const String kEndPointAdminAuth = "/api/users/login";
const String kEndPointSlider = "/api/sliders";

const String kEndPointFloor = "/api/floors";
const String kEndPointTables = "/api/tables";
const String kEndPointCategory = "/api/menucategories";
const String kEndPointItem = "/api/getitemsbycategory";
const String kEndPointSearchName = "/api/getItemBySearchKey";
const String kEndPointGetOrder = "/api/getorder";
const String kEndPointDeleteOrder = "/api/deleteorderitem";
const String kEndPointAddOrder = "/api/addorderitem";

const String NETWORK = 'Network';
const String BLUETOOTH = 'Bluetooth';

const String KITCHEN = 'Kitchen';
const String BBQ = 'BBQ';
const String BAR = 'Bar';
const String COUNTER = 'Counter';